...
class AC(nn.Module):
    def __init__(self, din, nk, nact):
        ...
        self.m = nn.Sequential(nn.Linear(din * 3, 256), nn.BatchNorm1d(256), nn.LeakyReLU(), nn.Linear(256, 256),
                               nn.BatchNorm1d(256), nn.LeakyReLU(), nn.Linear(256, nact))
...
class LatentForward(nn.Module):
    def __init__(self, dim, nact):
        super().__init__()
        self.net = nn.Sequential(nn.Linear(dim + nact, 256), nn.LeakyReLU(), nn.Linear(256, 256), nn.LeakyReLU(),
                                 nn.Linear(256, dim))
...
class Encoder(nn.Module):
    def __init__(self, din, dout):
        ...
        self.mixer = mixer.MLP_Mixer(n_layers=2, n_channel=32, n_hidden=32, n_output=256, image_size=100, patch_size=10,
                                     n_image_channel=3)
...
class Probe(nn.Module):
    def __init__(self, din, dout):
        ...
        self.enc = nn.Sequential(nn.Linear(din, 512), nn.LeakyReLU(), nn.Linear(512, 512), nn.LeakyReLU(),
                                 nn.Linear(512, dout))
