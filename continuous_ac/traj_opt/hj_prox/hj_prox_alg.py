...
class HJ_Prox_Optimizer:
    ...
    def grad_descent(self, iter_num = 10, x_init = None, t = None):
        ...
        for i in range(iter_num):
            ...
            print(f'grad desc. iter {i} takes {run_time} secs.')

            x = torch.clamp(prox, min = self.x_min, max = self.x_max) 
            sol_list.append(x)
...
class Tracking_Cost(nn.Module):
    ...
    #     # actions.size = (num_samples, n_batch, mpc_T, nu)
    #     num_samples = actions.size(0)
    #     costs = []
    #     for i in range(num_samples):
    #         cost = tracking_cost_helper(actions[i,:], self.forward_model, self.lat_state_init, self.lat_state_target)
    #         costs.append(cost)
...
def tracking_cost_helper_batch(actions, forward_model, lat_state_init, lat_state_target):
    ...
    for i in range(mpc_T):
        output = forward_model(lat_state.view(-1,nz), actions[:, :, i, :].view(-1, nu))
