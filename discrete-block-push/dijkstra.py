...
def step(counts, st, a):
    ns = counts[st][a]
    #return ns.argmax()

    ns_norm = ns/ns.sum()

    return numpy.random.multinomial(1, ns_norm*0.99).argmax()

def render(tup):
    print('Agent:(%d,%d).  Block:(%d,%d)' % (tup[0],tup[1],tup[2],tup[3]))

    agent_pos = [tup[0], tup[1]]
    block_pos = [tup[2], tup[3]]

    for j in range(6):
        for i in range(6):
            if agent_pos == [i,j] and block_pos == [i,j]:
                c = "X"
            elif agent_pos == [i,j]:
                c = 'x'                
            elif block_pos == [i,j]:
...
if __name__ == "__main__":
    ...
    adict = {0 : 'Left', 1: 'Right', 2: 'Up', 3: 'Down', 4: 'Stay'}

    import numpy as np

    import pickle

    c2s = pickle.load(open('dict.pkl', 'rb'))
    ...
    print('Goal: ')
    render(c2s[goal_index])

    for j in range(0, 25):
        d,g,a = DP_goals(ls,init_state, goal_index, dp_step=dp_step_use, code2ground={})

        print('State at step %d:' % j)
        render(c2s[init_state])
        ...
        print('Action: ', adict[a])
        #print('Goal: ', render(c2s[g]))
        #print('goal', g)

        init_state = step(counts, init_state, a)
        init_state = init_state.item()
